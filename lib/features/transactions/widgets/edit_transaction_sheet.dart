...
class _EditTransactionSheetState extends ConsumerState<EditTransactionSheet> {
  ...
  void initState() {
    ...
    _target = widget.transaction.targetModule;
    
    // Parse concept and optional note if written as "Concepto (Nota)"
    final origConcept = widget.transaction.concept;
    final noteRegExp = RegExp(r'\(([^)]+)\)$');
    final noteMatch = noteRegExp.firstMatch(origConcept);
    if (noteMatch != null) {
      _noteController.text = noteMatch.group(1) ?? '';
      _conceptController.text = origConcept.substring(0, noteMatch.start).trim();
    } else {
      _conceptController.text = origConcept;
    }

    _amountController.text = widget.transaction.amount.toStringAsFixed(2);
    _speech = stt.SpeechToText();

    // Determine visual category matching
    final origCategory = widget.transaction.category;
    List<CategoryItem> activeList;
    if (_type == TransactionType.gasto) {
      activeList = _target == TargetModule.casa ? _gastoCasaCategories : _gastoPersonalCategories;
    } else {
      activeList = _target == TargetModule.casa ? _abonoCasaCategories : _abonoPersonalCategories;
    }

    final isPredefined = activeList.any((item) => item.label == origCategory);
    if (isPredefined) {
      _selectedCategory = origCategory;
    } else {
      _selectedCategory = _type == TransactionType.gasto ? 'Otros' : 'Otro';
      _categoryController.text = origCategory;
    }
  }
